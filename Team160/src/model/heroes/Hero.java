...
public abstract class Hero implements MinionListener {
	...
	public String toString() {
		return   "HP : " + currentHP
				+ "\n\nManaCrystals "  + "\n"+currentManaCrystals+"/"+totalManaCrystals
				  + "\n\nCards Left in \n Deck : " + deck.size();
	}
	public String spelltoString(){
		String s = "YOUR SPELL'S DESCRIPTION";
		boolean cow = false;
		boolean ds = false;
		boolean fs = false;
		boolean hn = false;
	 ...
	 for(int i = 0 ; i<hand.size();i++){
	    if(hand.get(i) instanceof Spell ){
        	...
        	if(hand.get(i).getName().equals("Curse of Weakness")&& !cow){
        		s+="\n\nCurse of Weakness\n Decreases the attack of\nall enemy minions by 2. ";
        		cow = true;
        	}
        	if(hand.get(i).getName().equals("Divine Spirit")&& !ds){
        		s+="\n\nDivine Spirit\n Doubles the current \nand max HP of a minion.";
        		ds = true;
        	}
        	if(hand.get(i).getName().equals("Flamestrike")&& !fs){
        		s+="\n\nFlamestrike\nDeals 4 damage to all \nenemy minions.";
        		fs = true;
        	}
            if(hand.get(i).getName().equals("Holy Nova") && !hn){
            	s+="\n\nHoly Nova\nDeals 2 damage to all \nenemy minions.";
            	hn = true ;
            }
        	if(hand.get(i).getName().equals("Kill Command")&& !kc){
        		s+="\n\nKill Command\nDeals 5 damage to a \nminion or 3 damage to a hero.";
        		kc = true;
        	}
        	if(hand.get(i).getName().equals("Level Up!") && !lu){
        		s+="\n\nLevel Up\nIncrease the attack, current,\nand max HP of all silver hand recruits by 1";
        		lu = true;
        	}
            if(hand.get(i).getName().equals("Multi-Shot")&& !ms){
            	s+="\n\nMultishot\nDeals 3 damage to two \nrandom enemy minions.";
            	ms = true;
            }
            if(hand.get(i).getName().equals("Polymorph")&& !morph){
            	s+="\n\nPolymorph\nTransforms a minion into \na sheep minion";
            	morph = true;
            }
            if(hand.get(i).getName().equals("Pyroblast") && !pyro){
            	s+="\n\nPyroblast\nDeals 10 damage to \na chosen target";
            	pyro = true;
            }
            if(hand.get(i).getName().equals("Seal of Champions")&& !soc){
            	s+="\n\nSeal Of Champions\nIncreases the attack of a \nminion by 3 and gives it divine shield.";
            	soc = true;
            }
           if(hand.get(i).getName().equals("Shadow Word: Death")&& !swd){
            	s+="\n\nShadow Word:Death\nDestroys a minion that \nhis attack is 5 or more even if it's divine.";
            	swd = true ;
           }
            if(hand.get(i).getName().equals("Siphon Soul")&& !ss){
            	s+="\n\nSiphon Soul\nDestroys a minion even if it's \ndivine and restores 3 health points to the hero.";
            	ss = true;
            }
            if(hand.get(i).getName().equals("Twisting Nether") && !tn){
            	s+="\n\nTwisting nether\nDestroys all minions of \nboth heroes even if any of is divine.";
            	tn = true; 	
            }
        }
	 }
